...
def sendMail(self, emailText, subject, emails):
  ...
  try:
    ...
    print 'failed to send mail, because ',str(ex)
...
def connectRR_v3_API(runType, startRun, endRun, runDuration):
  ...
  runInfo.sort()
  good_bad_runs_list = inspect_run_goodnes(runInfo,0)
  messageToSent = ''
  
  if good_bad_runs_list[1]:
    messageToSent = 'Runs '
    for i in good_bad_runs_list[1]:
      messageToSent += str(i)+', '
    messageToSent += ' marked as good on RR but system on STDBY for > 10 % of all LS'
    #print messageToSent
    #sendMail('',messageToSent,'suspicious runs marked as good',arrayOfEmails)
      

  #print 'GOOD RUNS are ',good_bad_runs_list[0]
  #print 'BAD RUNS are ',good_bad_runs_list[1]
  return good_bad_runs_list[0]
...
def contDB(runOK,opt):
  if opt is 1:  #cms_orcoff_prep
    ...
    exitcode = subprocess.call("python runDataToDB_roll.py -o cms_orcoff_prep -u CMS_COND_RPC_NOISE -p j6XFEznqH9f92WUf run%s/database_new.txt" %runOK, shell=True)
    if exitcode != 0:
      print "ERROR!!!!!!!!!! runDataToDB_roll.py with %s doesn't work properly." %runOK
      return "ERROR"
    exitcode = subprocess.call("python runDataToDB_strip.py -o cms_orcoff_prep -u CMS_COND_RPC_NOISE -p j6XFEznqH9f92WUf run%s/database_full.txt" %runOK, shell=True)
    if exitcode != 0:
      print "ERROR!!!!!!!!!! runDataToDB_strip.py with %s doesn't work properly." %runOK
      return "ERROR"
      
  elif opt is 2:  #OMDS
    acc = 'oracle://cms_omds_lb/CMS_RPC_COND'
    auth = '/nfshome0/popcondev/conddb_taskWriters/RPC/authentication.xml'
    #'/nfshome0/popcondev/conddb/authentication.xml'
...
if runOptions[0] == 55:  # Just get runnumbers from RR
  ...
  if runOptions[0] == 2:
    for idx in range(0,len(runInfo)):
      #run = runNoise(runInfo[idx])
      run = runInfo[idx] # use this for runs with error codes
