...
out = commands.getoutput("ldd /nfshome0/mmaggi/PYTHON/cx_Oracle.so")
...
class DBService(object):
  def __init__(self, dbType='sqlite:///', host=None, port=None, user='', password= '', schema='', dbName='runData.db'):
    ...
    self.__engine = sqlalchemy.create_engine(self.__alchemyDBString)#, echo=True)
...
#    sqlalchemy.Column('rate_hz', sqlalchemy.Float(precision=10)),
  ...
  def deleteFromDB(self, runNumber=None):
    metadata = sqlalchemy.MetaData()
    table = sqlalchemy.Table('RPC_NOISE_ROLLS', metadata, schema=self.__schema,
...
#          exec_date=data['date'],
          raw_id=data['rawId'],
#          roll=data['rollName'],
          dead_strips=data['deadStrips'],
          masked_strips=data['maskedStrips'],
          strips_to_unmask=data['stripsToUnmask'],
          strips_to_mask=data['stripsToMask'], 
...
class FileReader(object):
  def __init__(self):
    ...
    #-99 : these chambers are LV/HV off
    self.__runDataPatternNon = """
      ^
      ([0-9]+[ \t]+){1}
      [a-zA-Z0-9_\+\-]+[ \t]+
      (-99[ \t]+){5}
  ...
  def __contentCheck(self, contents):
    ...
    for run in contents[1:]:
      if not re.match(self.__runDataPattern, run, re.VERBOSE):
        if not re.match(self.__runDataPatternNon, run, re.VERBOSE):
          contentMeta['correct'] = False
...
#      runData['rollName'] = data[1]
      runData['deadStrips'] = data[2]
      runData['maskedStrips'] = data[3]
      runData['stripsToUnmask'] = data[4]
      runData['stripsToMask'] = data[5]
...
class ArgumentParser(object):
  def __init__(self):
    ...
    usageStr += sys.argv[0] + ' -o oracleTNS -u userName -p password -S schemaname runDir\n'
...
#  dbService.createDB()
  ...
  print 'runDataToDB_roll.py: DONE Successfully'
  sys.exit(0) 
